...
<body>
        ...
        <form action="{{ route('siswa.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="display: flex; justify-content: space-between;">
                ...
                <div style="flex: 1; margin-right: 20px;">
                    ...
                    <input type="text" name="pendaftaran_id" value="{{ $kode }}" maxlength="70"
                    ...
                    <div style="display: flex; align-items: center;">
                        <div style="margin-right: 10px;">
                            <label>RT:</label>
                            <input type="text" name="rt" maxlength="3" style="width: 50px; margin-bottom: 5px;" oninput="validatenumber(this)">
                        </div>
                        <div style="margin-right: 10px;">
                            <label>RW:</label>
                            <input type="text" name="rw" maxlength="3" style="width: 50px; margin-bottom: 5px;" oninput="validatenumber(this)">
                        </div>
                    </div><br>
                ...
                <div style="flex: 1; margin-right: 20px;">
                    ...
                    <div style="display: flex; align-items: center;">
                        <div style="margin-right: 10px;">
                            <label>RT:</label>
                            <input type="number" name="rt_orang_tua" maxlength="3" style="width: 30px; margin-bottom: 5px;" oninput="validatenumber(this)">
                        </div>
                        <div style="margin-right: 10px;">
                            <label>RW:</label>
                            <input type="number" name="rw_orang_tua" maxlength="3" style="width: 30px; margin-bottom: 5px;" oninput="validatenumber(this)">
                        </div>
                    </div><br>
                    
                    <div style="display: flex; align-items: center;">
                        <div style="margin-right: 10px;">
                            <select id="provinsi_orang_tua" required style="width: 150px;">
                                <option value="">Pilih Provinsi</option>
                            </select>
                         </div>
                        <div style="margin-right: 10px;">
                            <select id="kota_orang_tua" disabled style="width: 200px;">
                                <option value="">Pilih Kota</option>
                            </select>
                        </div>
                    </div><br>
                    <div style="display: flex; align-items: center;">
                        <div style="margin-right: 10px;">
                            <select id="kecamatan_orang_tua" disabled style="width: 150px;">
                                <option value="">Pilih Kecamatan</option>
                            </select>                        
                         </div>
                        <div style="margin-right: 10px;">
                            <select id="kelurahan_orang_tua" disabled style="width: 150px;">
                                <option value="">Pilih Kelurahan</option>
                            </select>   
                        </div>
                    </div><br>
                ...
                <script>
                    ...
                    $(document).ready(function () {
                        fetch('https://raw.githubusercontent.com/MuhammadSheva99/ppdb-reg/refs/heads/main/indonesia-region.min.json')
            ...
            <script>
                console.log("Data Regions:", regionsData);


                $(document).ready(function () {
                    // Muat data JSON dari URL
                    fetch('https://raw.githubusercontent.com/MuhammadSheva99/ppdb-reg/refs/heads/main/indonesia-region.min.json')
                        .then(response => response.json())
                        .then(data => {
                            regionsData = data;

                            console.log("Mengisi dropdown provinsi orang tua");
                            Object.keys(regionsData).forEach(provinsiId => {
                                const provinsiName = regionsData[provinsiId].name;
                                console.log(`Provinsi: ${provinsiName}`);
                                $('#provinsi_orang_tua').append(`<option value="${provinsiId}">${provinsiName}</option>`);
                            });
                        })
                        .catch(error => console.error('Error loading regions data:', error));
                });
                ...
                $('#provinsi_orang_tua').change(function () {
                    const provinsiId = $(this).val();
                    $('#kota_orang_tua').html('<option value="">Pilih Kota</option>').prop('disabled', true);
                    $('#kecamatan_orang_tua').html('<option value="">Pilih Kecamatan</option>').prop('disabled', true);
                    $('#kelurahan_orang_tua').html('<option value="">Pilih Kelurahan</option>').prop('disabled', true);

                    if (provinsiId) {
                        const cities = regionsData[provinsiId].regencies || {};
                        $('#kota_orang_tua').prop('disabled', false);

                        Object.keys(cities).forEach(kotaId => {
                            const kotaName = cities[kotaId].name;
                            $('#kota_orang_tua').append(`<option value="${kotaId}">${kotaName}</option>`);
                        });
                    }
                });

                // Saat kota dipilih
                $('#kota_orang_tua').change(function () {
                    const provinsiId = $('#provinsi_orang_tua').val();
                    const kotaId = $(this).val();
                    $('#kecamatan_orang_tua').html('<option value="">Pilih Kecamatan</option>').prop('disabled', true);
                    $('#kelurahan_orang_tua').html('<option value="">Pilih Kelurahan</option>').prop('disabled', true);
                    ...
                    if (kotaId) {
                        const districts = regionsData[provinsiId].regencies[kotaId].districts || {};
                        $('#kecamatan_orang_tua').prop('disabled', false);

                        Object.keys(districts).forEach(kecamatanId => {
                            const kecamatanName = districts[kecamatanId].name;
                            $('#kecamatan_orang_tua').append(`<option value="${kecamatanId}">${kecamatanName}</option>`);
                        });
                    }
                });

                // Saat kecamatan dipilih
                $('#kecamatan_orang_tua').change(function () {
                    const provinsiId = $('#provinsi_orang_tua').val();
                    const kotaId = $('#kota_orang_tua').val();
                    const kecamatanId = $(this).val();
                    $('#kelurahan_orang_tua').html('<option value="">Pilih Kelurahan</option>').prop('disabled', true);

                    if (kecamatanId) {
                        const villages = regionsData[provinsiId].regencies[kotaId].districts[kecamatanId].villages || {};
                        $('#kelurahan_orang_tua').prop('disabled', false);

                        Object.keys(villages).forEach(kelurahanId => {
                            const kelurahanName = villages[kelurahanId].name;
                            $('#kelurahan_orang_tua').append(`<option value="${kelurahanId}">${kelurahanName}</option>`);
                        });
                    }
                });
    ...
    <script>
        function showPreview() {
            ...
            const daftar = document.querySelector('input[name="nomor_pendaftaran"]').value || "Tidak ada data";
            ...
            const provinsiOrangtua = document.getElementById("provinsi_orang_tua").name;
            const kotaOrangtua = document.getElementById("kota_orang_tua");
            const kecamatanOrangtua = document.getElementById("kecamatan_orang_tua");
            const kelurahanOrangtua = document.getElementById("kelurahan_orang_tua");
                ...
                <div class="container">
                    <div class="foto">
                     ...
                     <div class="info">
                        ...
                        <div class="info-row"><div class="label">Daftar Siswa</div><div class="separator">:</div><div class="value ${daftar === 'Tidak ada data' ? 'red-text' : ''}">${daftar}</div></div>
                       ...
                       <br>
                        ...
                        <div class="info-row"><div class="label">provinsi Orang Tua</div><div class="separator">:</div><div class="value ${provinsiOrangtua === 'Tidak ada data' ? 'red-text' : ''}">${provinsiOrangtua}</div></div>
                        <div class="info-row"><div class="label">kota</div><div class="separator">:</div><div class="value ${kotaOrangtua === 'Tidak ada data' ? 'red-text' : ''}">${kotaOrangtua}</div></div>
                        <div class="info-row"><div class="label">kecamatan</div><div class="separator">:</div><div class="value ${kecamatanOrangtua === 'Tidak ada data' ? 'red-text' : ''}">${kecamatanOrangtua}</div></div>
                        <div class="info-row"><div class="label">kelurahan</div><div class="separator">:</div><div class="value ${kelurahanOrangtua === 'Tidak ada data' ? 'red-text' : ''}">${kelurahanOrangtua}</div></div>
